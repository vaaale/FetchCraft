...
async def rag_pipeline_example():
    ...
    for text in documents_text:
        node = Node(
            text=text,
            metadata={"parsing": "example", "type": "document"}
        )
        nodes.append(node)
    ...
    doc_ids = await index.add_nodes(DocumentNode, nodes)
...
async def document_parsing_with_embeddings():
    ...
    # Load document
    source = FilesystemDocumentParser.from_file(sample_file)
    ...
    chunk_ids = await index.add_nodes(DocumentNode, chunks)
