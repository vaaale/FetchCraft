...
class PDFDocumentParser(DocumentParser):
    """
    Document parsing for parsing PDF files using pymupdf4llm.
    
    This parsing extracts text from PDFs with markdown formatting,
    preserving document structure, tables, and other formatting.
    
    Example:
        ```python
        # Load single PDF file
        parsing = PDFDocumentSource.from_file(Path("document.pdf"))
        
        # Load all PDFs from directory
        parsing = PDFDocumentSource.from_directory(
            Path("./pdfs"), 
            recursive=True
        )
