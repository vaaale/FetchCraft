...
class RemoteDoclingParser(DocumentParser):
    ...
    timeout: int = 60 * 40
    foo_callback_url: Optional[str] = None
    ...
    def __init__(
        ...
        super().__init__(
            ...
            client=client, 
            timeout=timeout,
            foo_callback_url=callback_url,
        )

    @property
    def callback_url(self):
        return self.foo_callback_url
    ...
    async def parse(self, file: File, metadata: Optional[Dict[str, Any]] = None, task_id: Optional[str] = None, **parser_kwargs) -> AsyncGenerator[DocumentNode, None]:
        ...
        In async mode (when foo_callback_url is set):
        ...
        with tempfile.NamedTemporaryFile(delete=True, suffix=file_suffix) as tmp:
            ...
            # If foo_callback_url is set, use async mode
            if self.foo_callback_url:
                # Submit job asynchronously with callback and task_id
                response = await self.client.submit_job(
                    tmp.name, 
                    callback_url=self.foo_callback_url,
                    task_id=task_id
                )
