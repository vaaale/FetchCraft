...
async def test_vector_store_skips_unchanged_documents(mock_embeddings):
    ...
    await index.add_documents([node1])
    
    # Retrieve it to verify it was added
    result = await vector_store.get_document(doc_id, index_id="test")
    ...
    await index.add_documents([node2])
    
    # Retrieve it - should still be the same
    result2 = await vector_store.get_document(doc_id, index_id="test")
...
async def test_hash_automatically_computed_if_missing(mock_embeddings):
    ...
    await index.add_documents([node])
