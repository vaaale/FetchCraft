...
async def test_node_hash_computation():
    ...
    node1 = Node(text="Hello world", metadata={"parsing": "test"})

    assert node1.doc_hash is not None
    assert len(node1.doc_hash) == 32  # MD5 hash is 32 hex chars

    # Same content should produce same hash
    node2 = Node(text="Hello world", metadata={"parsing": "test"})
...
async def test_node_hash_differs_with_different_content():
    ...
    node2 = Node(text="Different text", metadata={"parsing": "test"})
...
async def test_node_hash_differs_with_different_metadata():
    ...
    node1 = Node(text="Hello world", metadata={"parsing": "test1"})

    node2 = Node(text="Hello world", metadata={"parsing": "test2"})
...
async def test_vector_store_skips_unchanged_documents(mock_embeddings):
    ...
    doc_id = str(uuid4())
    node1 = Node(id=doc_id, text="Hello world", metadata={"parsing": "test"})
    ...
    node2 = Node(id=doc_id, text="Hello world", metadata={"parsing": "test"})
...
async def test_vector_store_updates_changed_documents(mock_embeddings):
    ...
    node2 = Node(id=doc_id, text="Hello world updated!", metadata={"parsing": "test"})
...
async def test_hash_automatically_computed_if_missing(mock_embeddings):
    ...
    doc_id = str(uuid4())
    node = Node(id=doc_id, text="Hello world", metadata={"parsing": "test"})
