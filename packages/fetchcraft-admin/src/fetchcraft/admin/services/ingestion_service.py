...
class IngestionService:
    ...
    async def create_job(
        ...
        asyncio.create_task(self._run_job(pipeline))
    ...
    async def recover_running_jobs(
        ...
        for job in jobs_to_recover:
            try:
                ...
                asyncio.create_task(self._recover_job(pipeline))
    ...
    async def _recover_job(self, pipeline: TrackedIngestionPipeline):
        ...
        """
        try:
            logger.info(f"Starting workers for job recovery: {pipeline.job.name}")
            ...
            logger.info(f"Job '{pipeline.job.name}' recovered and completed")
            
        except Exception as e:
            logger.error(f"Job '{pipeline.job.name}' recovery failed: {e}", exc_info=True)
